...
def read_dataset(filename):
    ...
    def parse_fn(example_proto):
        ...
        sequence_features = {
            'inputs': tf.FixedLenSequenceFeature(shape=[], dtype=tf.string),
...
def process_dataset(dataset, vocab_table, sos, eos,
    ...
    dataset = dataset.map(
        lambda inputs, labels: (vocab_table.lookup(inputs),
                                vocab_table.lookup(labels)),
        num_parallel_calls=num_parallel_calls)

    dataset = dataset.map(
        lambda inputs, labels: (tf.cast(inputs, tf.int32),
